...
class Decoder:
    ...
    def _next_token(self, input_ids: list[int], allowed_tokens: list[int]) -> int:
    ...
    def _extract_parameters(self, text: str, fn: FunctionDefinition) -> dict:
        ...
        paths = re.findall(r"(\/[\w\/\.\-]+|[A-Za-z]:\\\\[\w\\\\\.\-]+)",
                           text)

        path_index = 0

        # Extract SQL queries
        sql = re.findall(r"(SELECT .*?;|INSERT .*?;|UPDATE .*?;|DELETE .*?;)", text, re.IGNORECASE)
        sql_index = 0
        ...
        for name, param in fn.parameters.items():
            ...
            # -----------------------------
                ...
                # 4. Fallback: last word (NOT entire prompt)
